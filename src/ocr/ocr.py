...
class OcrProcessor(threading.Thread):
    ...
    def run(self):
        ...
        while self.shared_state.running:
            try:
                ...
                # Re-trigger immediately so hit_scan runs at current mouse pos
    ...
    def _discover_providers(self) -> Dict[str, Type[OcrProvider]]:
        ...
        source_path = Path(__file__).parent / 'providers'
        frozen_path = Path(getattr(sys, '_MEIPASS', '')) / 'src' / 'ocr' / 'providers'
        providers_path = frozen_path if frozen_path.exists() else source_path
